...
vol_info=($(flyctl volumes list -a ${FLY_APP_NAME}-db | grep GB))
...
if [ -z $(awk -v db_s=$db_size_gb -v vol_s=$vol_size_gb \
    ...
    flyctl volumes create pg_data \
       ...
       flyctl scale count 2 --app ${FLY_APP_NAME}-db

       # Wait for the second instance finish syncing and adopt the "replica" status
      while [ -z "$(flyctl status --app ${FLY_APP_NAME}-db | grep replica)" ]
      do
        sleep 15
      done
      ...
      flyctl scale count 1 --app ${FLY_APP_NAME}-db

      # Wait for the new instance to adopt the "leader" status
      while [ -z "$(flyctl status --app ${FLY_APP_NAME}-db | grep leader)" ]
      do
        sleep 15
      done
